...
class Transformer(nn.Module):
    ...
    def encode(self, train_feat):
        ...
        if self.feature_adjustor is not None:
            print('train_feat_adj1:',train_feat.view(-1, *train_feat.shape[-3:]).shape)
    ...
    def forward(self, test_feat, train_feat, train_mask=None, train_mask_enc=None):
        ...
        # print('encoded_train_feat.shape:','----',encoded_train_feat.shape)
...
if __name__ == "__main__":
    transformer_feature_adjustor = nn.Conv2d(512, 1024, 1, 1)
    transformer_feature_extractor = nn.Conv2d(512, 512, 1, 1)

    tfm = Transformer(512, 128, feature_adjustor=None, feature_extractor=None)
    # tfm.cuda()
    train_feat = torch.rand(2, 3, 512, 56, 30)
    # .cuda()
    train_mask_enc = torch.rand(2, 3, 1, 56, 30)
    # .cuda()
    test_feat = torch.rand(1, 3, 512, 56, 30)
    # .cuda()
    # test_mask = torch.rand(1, 2, 56*16, 30*16)
    # .cuda()
    with torch.no_grad():
        # tfm.init(test_feat, test_mask)
        decoded_mask_enc = tfm(test_feat, train_feat, train_mask_enc=train_mask_enc)
        # print(decoded_mask_enc.shape)
